...
def eval(config, test_env):
    ...
    def run(rng, params):
        config["NUM_ACTORS"] = test_env.num_agents * config["NUM_ENVS"]
        config["NUM_TEST_ACTORS"] = test_env.num_agents * config["NUM_TEST_ENVS"]
        config["NUM_UPDATES"] = (
        ...
        config["CLIP_EPS"] = (
            config["CLIP_EPS"] / test_env.num_agents
            if config["SCALE_CLIP_EPS"]
            else config["CLIP_EPS"]
        )
        config["FC_DIM_SIZE"] = config["HIDDEN_SIZE"]
        config["GRU_HIDDEN_DIM"] = config["HIDDEN_SIZE"]


        # INIT ENV
        rng, _rng = jax.random.split(rng)
        reset_rng = jax.random.split(_rng, config["NUM_ENVS"])
        obsv, env_state = jax.vmap(test_env.reset, in_axes=(0,))(reset_rng)
        ...
        network = ActorCriticRNN(test_env.action_space(test_env.agents[0]).n, config=config)
        ...
        def _get_greedy_metrics(rng, actor_params):
            ...
            def _greedy_env_step(step_state, unused):
                ...
                # SELECT ACTION
                rng, _rng = jax.random.split(rng)
                obs_batch = batchify(last_obs, test_env.agents, config["NUM_TEST_ACTORS"])
                ac_in = (
                    obs_batch[np.newaxis, :],
                    last_done[np.newaxis, :],
                )
                ac_hstate, pi, _ = network.apply(actor_params, ac_hstate, ac_in)
                # here, instead of sampling from distribution, take mode
                action = pi.mode()
                env_act = unbatchify(
                    action, test_env.agents, config["NUM_TEST_ENVS"], test_env.num_agents
                )
                ...
                done_batch = batchify(done, test_env.agents, config["NUM_TEST_ACTORS"]).squeeze()
                reward_batch = batchify(reward, test_env.agents, config["NUM_TEST_ACTORS"]).squeeze()
...
def single_run(config):
    ...
    alg_name = config.get("ALG_NAME", "ippo")
